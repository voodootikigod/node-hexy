...
module.exports = function (serialport_target, cb) {
	...
	that.allParts = function (item) {
		var parts = [];
		['left', 'right'].forEach(function (side) {
			['front', 'middle', 'back'].forEach(function (position) {
				parts.push(that.hexapod.legs[side][position][item]);
			});
		})
		return parts;
	}
	...
	that.ankles = function() {
		return that.allParts('ankle');
	}
	that.hips = function() {
		return that.allParts('hip');
	}
	that.knees = function() {
		return that.allParts('knee');
	}
	...
	that.up = function (done) {
		...
		var actions = [
		  ...
		  // line 17
		  {
		    delay: 500,
		    task: function() {
		    	that.knees().map(function (ankle) { console.log(ankle); ankle.setPos(-30); });
		    	that.hips().map(function (hip) { hip.kill(); });
		    }
		  },
		  // line 23
		  {
		    delay: 500,
		    task: function() {
		    	that.ankles().map(function (ankle) { ankle.setPos(-90); });
		    }
		  }
		];
		...
		// line 28
		[0, 3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36, 39, 42].forEach(function(angle) {
			actions.push({
				delay: 100,
				task: function () {
		    	that.knees().map(function (knee) { knee.setPos(angle); });
		    	that.ankles().map(function (ankle) { ankle.setPos(-90+angle); });
				}
			});
		});
